...
class ContentValidator {
  ...
  ValidationResult validateFile(String filePath) {
    ...
    try {
      ...
      if (content is Map<String, dynamic>) {
        if (content.containsKey('topic_id') &&
            content.containsKey('synopsis_cards')) {
        ...
        if (content.containsKey('patterns') ||
            content.containsKey('pattern_metadata')) {
      ...
      if (question is Map<String, dynamic>) {
        ...
        for (final field in requiredFields) {
          if (!question.containsKey(field) || question[field] == null) {
            errors.add(
                'Question $i missing required field "$field" in $filePath');
          }
        }
