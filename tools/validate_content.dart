...
class ContentValidator {
  ...
  ValidationResult _validateContent(Map<String, dynamic> content, String filePath) {
    _errors.clear();
    _warnings.clear();
  ...
  void _validateBusinessRules(Map<String, dynamic> content, String filePath) {
    ...
    if (content['primary_concept'] == null || content['primary_concept'].toString().isEmpty) {
    ...
    if (content['question_type'] == 'multiple_choice') {
      ...
      if (whyWrong == null || whyWrong.length < 3) {
        _errors.add('MCQ must have why-wrong explanations for at least 3 incorrect options in $filePath');
      }
    }
    
    // Rule 4: Diagram questions must have valid diagram_id
    if (content['diagram_required'] == true) {
      if (content['diagram_id'] == null || content['diagram_id'].toString().isEmpty) {
        _errors.add('Diagram required but no diagram_id provided in $filePath');
      }
      
      if (content['diagram_specification'] == null) {
        _warnings.add('Diagram required but no specification provided in $filePath');
      }
    }
    ...
    if (content['review_status'] == 'published') {
      ...
      if (reviewHistory == null || reviewHistory.isEmpty) {
        _warnings.add('Published content should have review_history in $filePath');
      }
    }
    ...
    if (expectedTime != null) {
      if (expectedTime < 30 || expectedTime > 300) {
        _warnings.add('Expected time $expectedTime seconds is outside recommended range (30-300) in $filePath');
      }
    }
    ...
    if (content['bridge_level'] == 'jee') {
      ...
      if (rescueIds == null || rescueIds.length < 2) {
        _errors.add('JEE question must have at least 2 rescue question IDs in $filePath');
      }
      ...
      if (rescueIds != null) {
        for (final rescueId in rescueIds) {
          ...
          if (!validPattern.hasMatch(rescueId)) {
            _errors.add('Invalid rescue question ID format: $rescueId in $filePath');
          }
        }
      }
    }
    ...
    if (classLevel != null && bridgeLevel != null) {
      if (bridgeLevel == 'jee' && !classLevel.contains('11') && !classLevel.contains('12')) {
        _warnings.add('JEE bridge level question should target Class 11-12 in $filePath');
      }
      
      if (bridgeLevel == 'foundation' && (classLevel.contains('11') || classLevel.contains('12'))) {
        _warnings.add('Foundation level question should target lower classes in $filePath');
      }
    }
    ...
    if (solutionSteps == null || solutionSteps.isEmpty) {
      ...
      for (int i = 0; i < solutionSteps.length; i++) {
        ...
        if (step['step_number'] == null) {
          _errors.add('Solution step missing step_number at index $i in $filePath');
        }
        
        if (step['description'] == null || step['description'].toString().isEmpty) {
          _errors.add('Solution step missing description at index $i in $filePath');
        }
      }
    }
  }
  ...
  ValidationResult validateDirectory(String directoryPath) {
    ...
    return ValidationResult(
      ...
      summary: 'Validated $totalCount files: $validCount valid, ${totalCount - validCount} invalid',
    );
  }
...
void main(List<String> arguments) {
  ...
  if (arguments.isEmpty) {
    print('Usage: dart run tools/validate_content.dart [file_path|directory_path|pipeline]');
    print('Examples:');
    print('  dart run tools/validate_content.dart content/sample_questions/question.json');
