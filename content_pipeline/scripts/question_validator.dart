...
class QuestionValidator {
  ...
  Future<void> validateQuestionFile(File file) async {
    ...
    try {
      ...
      if (totalErrors == 0) {
        ...
        print(
            '⚠️  Found $totalErrors validation issues across ${questions.length} questions');
      }
    } catch (e) {
  ...
  List<String> validateQuestion(Map<String, dynamic> question) {
    ...
    if (sourceType != null &&
      ...
      errors.add(
          'Invalid source_type: $sourceType. Must be original_recreated or original_authored');
    }
    ...
    if (correctAnswer != null && options != null) {
      if (correctAnswer < 0 || correctAnswer >= options.length) {
        errors.add(
            'correct_answer index $correctAnswer is out of range (0-${options.length - 1})');
      }
    }
    ...
    if (questionRole != null &&
        !['starter', 'practice', 'challenge', 'jee_style']
            .contains(questionRole)) {
  ...
  void validateNewQuestionFormat(
      Map<String, dynamic> question, List<String> errors) {
    final questionId = question['question_id'] as String?;

    // Validate question ID format
    if (questionId != null &&
        !questionId.startsWith('fundamental_') &&
        !questionId.startsWith('practice_') &&
        !questionId.startsWith('challenge_') &&
        !questionId.startsWith('jee_')) {
      errors.add(
          'Question ID should follow pattern: fundamental_, practice_, challenge_, or jee_');
    }
    ...
    if (manipulative != null && !validManipulatives.contains(manipulative)) {
      errors.add(
          'Unsupported manipulative: $manipulative. Valid: ${validManipulatives.join(', ')}');
    }
  }

  /// Validate content quality requirements
  void validateContentQuality(
      Map<String, dynamic> question, List<String> errors) {
    ...
    if (classLevel == 'Class 7' || classLevel == 'Class 8') {
      ...
      for (final term in jeeTerms) {
        if (questionText.toLowerCase().contains(term.toLowerCase())) {
          errors.add(
              'Question contains advanced JEE terminology inappropriate for $classLevel');
        }
      }
    }
    ...
    if (manipulative == 'none' && question['diagram_required'] == true) {
      errors.add(
          'Questions requiring diagrams should have appropriate manipulatives');
    }
  }

  /// Generate QA checklist for each question
  Future<void> generateQAChecklist(String filePath, List questions,
      Map<String, List<String>> validationResults) async {
    final fileName = filePath.split('/').last;
    final checklistName =
        fileName.replaceAll('_questions.json', '_qa_checklist.md');
    ...
    final totalErrors =
        validationResults.values.fold(0, (sum, errors) => sum + errors.length);
    final passedQuestions =
        validationResults.entries.where((entry) => entry.value.isEmpty).length;
    ...
    for (final question in questions) {
      ...
      buffer.writeln(
          '- **Status:** ${errors.isEmpty ? '✅ PASS' : '⚠️ NEEDS REVIEW'}');
    ...
    buffer
        .writeln('- ✅ Source type is original_recreated or original_authored');
    buffer.writeln('');
